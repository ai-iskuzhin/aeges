...
public sealed class AgentProcessManagerTests
{
    [Fact]
    public async Task GetStatusAsync_reports_not_running_without_metadata()
    {
        using var runtime = new TemporaryRuntimeDirectory();
        var status = await new AgentProcessManager(runtime.Layout).GetStatusAsync(CancellationToken.None);

        Assert.False(status.IsRunning);
        Assert.False(status.IsStale);
        Assert.Null(status.Metadata);
        Assert.EndsWith(Path.Combine("runs", "agent.pid.json"), status.MetadataPath);
    }
    ...
    private sealed class TemporaryRuntimeDirectory : IDisposable
    {
        ...
        public async Task WriteMetadataAsync(int processId)
        {
            var metadata = new AgentProcessMetadata(
                ...
                "aeges",
                ["agent", "run", "--no-interactive"],
                Path.Combine(Layout.LogsPath, "agent.stdout.log"),
                Path.Combine(Layout.LogsPath, "agent.stderr.log"),
                null,
                null);

            await using var stream = File.Create(Path.Combine(Layout.RunsPath, "agent.pid.json"));
